/*
 * $Id: wacom.c,v 1.5 2004-10-07 18:36:35 pingc Exp $
 *
 ...
 * Version Information
 */
#define DRIVER_VERSION "v1.30-j0.6.5"
...
static void wacom_intuos3_irq(struct urb *urb)
{
	...
	if ((data[1] & 0xfc) == 0xc0)
	{
		...
		#ifdef __JEJ_DEBUG
		printk(KERN_INFO "wacom_intuos3_irq: tool change 0x%03X\n",
				(((__u32)data[2] << 4) | (data[3] >> 4)));
		#endif
...
{
	unsigned char read, rdata[2], limit=0;
	#ifdef __JEJ_DEBUG
